...
namespace OnePortal.Helper
{
    public class NewProcurement
    {
        ...
        public void AddItemsToProcure(XElement definition, Step step, XElement definition_step, FormCollection collection, HRM.DAL.Models.admin_hrm_employee employee, Guid id)
        {
            ...
            var units = collection.GetValues("unit"); ////collection["unit"].ToString().Split(',');
            var quantities = collection.GetValues("quantity"); //collection["quantity"].ToString().Split(',');
            var descriptions = collection.GetValues("description"); //collection["description"].ToString().Split(',');
            var amounts = collection.GetValues("unit_cost"); //collection["unit_cost"].ToString().Split(',');
            var itemnames = collection.GetValues("itemname");
            /*
        ...
        public void CreateVPRF(XElement definition, Step step, XElement definition_step, FormCollection collection)
        {
            ...
            //definition.Element("form").Add(new XElement("new_description", collection["new_description"].ToString()));
            //definition.Element("form").Add(new XElement("date", date));
            ...
            payment_element.Add(new XElement("date", date));
            payment_element.Add(new XElement("new_description", collection["new_description"] != null ? collection["new_description"].ToString() : ""));
        ...
        public string Approve(XElement definition, Step step, XElement definition_step, FormCollection collection, HRM.DAL.Models.admin_hrm_employee employee, Guid id, bpm_workflow workflow)
        {
            ...
            {
                var a_token = _transactionTokenService.GetApprovalToken(approval_token);
        ...
        public void Complete(XElement definition, Step step, XElement definition_step, FormCollection collection, HRM.DAL.Models.admin_hrm_employee employee, Guid id, bpm_workflow workflow)
        {
            definition_step.Element("status").SetValue(1);
            workflow.status = 23;
        }
