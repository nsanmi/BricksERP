...
namespace OnePortal.Controllers
{
    ...
    public class WorkflowController : Controller
    {
        ...
        public WorkflowController(IProcessService processService, IWorkflowService workflowService, IEmployeeService employeeService, IVendorService vendorService, ITransactionTokenService transactionTokenService)
        {
        ...
        public ActionResult MyPendingApproval(int? page, string search = null)
        {
            ...
            ViewBag.page = pageIndex;
            ViewBag.search = search_query;
            return View(_workflowService.GetActionWorkflows(employee.emp_number).Where(e => e.bpm_workflow.title.Contains(search_query) && e.bpm_workflow.status == 22).OrderByDescending(e => e.created_at).ToPagedList(pageIndex, pageSize));
        }
        ...
        public ActionResult Step(Guid id, int? step_id, string message = null)
        {
            ...
            if (step_id.HasValue)
            {
                ...
                //workflow.status = 22;// Allows submit button to show 
                //ViewBag.workflow = workflow;
                //var current_step_element_tag = step.Definition;
                //var next_step_element_tag = current_step_element_tag.Element("next_step");
            ...
            else if (step.Id > 1)
            {
                ...
                {
                    case "sup":
                        //get the supervisor of the person that applied for the request and know if he/she is the current logged in user
                        ...
                        break;
                    case "jti":
                        //check if the current user job title is a GOAL
                        ...
                        {
                            if (job_title_id == employee.job_title_code)
                            {
                                user_can_perform_this_action = true;
                            }
                        }
            ...
            if (!user_can_perform_this_action)
            {
                ViewBag.message = "It is not yet your turn in this request flow, Please exercise patience.";
        ...
        {
            var user_id = User.Identity.GetUserId(); //get currently logged in user
            var loggedInEmployee = _employeeService.GetEmployeeByUserId(user_id); //get logged in user as employee
            int complete = 0; //this is used to mark the form as completed

            var id = new Guid(collection["workflow_id"]);
            var bLWorkflow = new BLWorkflow(id);

            var workflow = bLWorkflow.GetWorkflow();//get the db table class bpm_workflow for updating of status and possibly definition extraction
            var step = bLWorkflow.GetCurrentStep();//gets the step in a custom Step class

            var definition = bLWorkflow.GetDefinition();// gets the definition

            var skip = false;

            XElement definition_step = null;
            //this if and elseif check if the step is on Assign Officer. this is for only old procurement integration and sets skip to true to skip other stuff
            if (definition.Element("next_step").Value == "-1" && workflow.process_id.ToString().ToUpper() == "94E9FEBB-6C8E-42E2-B1A2-4DEE7F6E7317" && loggedInEmployee.emp_number == Util.GetProgramApprover(-1, 0))
            {
                //var project_d = Util.GetProgramApprover(project_id, 0);
            ...
            else if (workflow.process_id.ToString().ToUpper() == "2D008467-EA12-4821-A2F2-7568B76039F1" && definition.Element("next_step").Value == "-1" && loggedInEmployee.emp_number == Util.GetProgramApprover(-1, 0))
            {
                if (collection["wf_assignment"] != null && collection["wf_assignment"] == "emp")
                {
                    //step = bLWorkflow.GetStep(Convert.ToInt32(definition.Element("actual_step").Value));
            ...
            if (!skip)
            {
                ...
                {
                    //get the person making the request
                    if (loggedInEmployee.emp_number == workflow.created_by)
                    {
                        user_can_perform_this_action = true;
                    }
                }
                else if (step.Id > 1)
                {
                    ...
                    switch (position.Attribute("pos_type").Value)
                    {
                        case "sup":
                            ...
                            var requestor_no = workflow.admin_hrm_employee.emp_number;
                            if (loggedInEmployee.admin_hrm_emp_reportto.FirstOrDefault(e => e.erep_sub_emp_number == requestor_no) != null)
                            {
                                user_can_perform_this_action = true;
                            }
                            break;
                        case "jti":
                            var job_title_id = Convert.ToInt32(position.Value);
                            if (job_title_id == loggedInEmployee.admin_hrm_emp_job_record.job_title_id)
                            {
                                user_can_perform_this_action = true;
                            }
                            break;
            ...
            {
                RefreshToken(loggedInEmployee, collection, id);
            ...
            if (!skip)
            {
                ...
                if (workflow.process_id.ToString().ToUpper() == "94E9FEBB-6C8E-42E2-B1A2-4DEE7F6E7317" || workflow.process_id.ToString().ToUpper() == "10471746-0236-4F79-BE27-8AA1CBFF2D92" || workflow.process_id.ToString().ToUpper() == "396E4066-81F1-4311-B076-FC3C53BB3178")
                {
                    ...
                    if (step.Id == 1)
                    {
                        procurement.AddItemsToProcure(definition, step, definition_step, collection, loggedInEmployee, id);
                    ...
                    {
                        var result = procurement.Approve(definition, step, definition_step, collection, loggedInEmployee, id, workflow);
                        if (result != "success")
                        {
                    ...
                    else if (step.Id == 8)
                    {
                        ...
                        RFQ(workflow, collection, definition, loggedInEmployee, step, definition_step);
                    ...
                    {
                        BidComparison(workflow, collection, definition, loggedInEmployee, step, definition_step);
                    ...
                    {
                        var result = HQApproval(workflow, collection, definition, step, definition_step, loggedInEmployee);
                        if (result != "success")
                        {
                            ViewBag.message = "Your token is expired. Please click refresh to generate a new one.";
                            return RedirectToAction("Step", new { id = workflow.id, message = result });
                ...
                else if (workflow.process_id.ToString().ToUpper() == "A9BE0559-94E4-4A6E-AE46-74A1344DF7DF" || workflow.process_id.ToString().ToUpper() == "A9BE0559-94E4-4A6E-AE46-74A1344DF7EF")
                {
                    ...
                    if (step.Id == 1)
                    {
                        ...
                        signature_element.Add(new XElement("name", string.Format("{0} {1}", loggedInEmployee.emp_lastname, loggedInEmployee.emp_firstname)));
                        signature_element.Add(new XElement("emp_number", loggedInEmployee.emp_number));
                    ...
                    else if (step.Id == 3)
                    {
                        ...
                        if (travel_expenses.Any())
                        {
                            ...
                            foreach (var expense in travel_expenses)
                            {
                                ...
                                if (!string.IsNullOrEmpty(collection[expense.expense_id + "_start_date"])
                                    && !string.IsNullOrEmpty(collection[expense.expense_id + "_end_date"])
                                    && !string.IsNullOrEmpty(collection[expense.expense_id + "_no_of_days"]))
                                {
                        ...
                        {
                            if (!string.IsNullOrEmpty(collection["h_start_date"])
                                && !string.IsNullOrEmpty(collection["h_end_date"])
                                && !string.IsNullOrEmpty(collection["h_no_of_days"]))
                            {
                                var element = new XElement("expense");
                        ...
                        //check for hotel
                        if (collection["transportation"] != null)
                        {
                            if (!string.IsNullOrEmpty(collection["t_start_date"])
                                && !string.IsNullOrEmpty(collection["t_end_date"])
                                && !string.IsNullOrEmpty(collection["t_no_of_days"]))
                            {
                                var element = new XElement("expense");
                        ...
                        if ((collection["other"] != null && collection["other"].ToString() != "") && (collection["other_amount"] != null && collection["other_amount"] != ""))
                        {
                            var others = collection["other"].Split(',');
                            ...
                            if (others.Count() == other_amounts.Count())
                            {
                                ...
                                {
                                    if (string.IsNullOrEmpty(others[i].Trim())
                                        || string.IsNullOrEmpty(other_amounts[i].Trim())
                                        || string.IsNullOrEmpty(other_ends[i].Trim())
                                        || string.IsNullOrEmpty(other_starts[i].Trim())
                                        || string.IsNullOrEmpty(days[i].Trim())) continue;
                                    var element = new XElement("expense");
                        ...
                        //now check if the form is actually empty here
                        var filled_data = definition.Element("form").Element("expenses");
                        if (!filled_data.HasElements)
                        {
                            ViewBag.message = "Check and fill the Advance form properly, make sure you enter the dates and no of days";
                            return RedirectToAction("Step", new { id = workflow.id, message = ViewBag.message });
                        }
                    ...
                    if (workflow.process_id.ToString().ToUpper() == "A9BE0559-94E4-4A6E-AE46-74A1344DF7EF")
                    {
                        //if (step.Id == 2 || step.Id == 4 || step.Id == 5 || step.Id == 6 || step.Id == 7)
                        //{
                        //    var result = new Procurement().Approve(definition, step, definition_step, collection, loggedInEmployee, id, workflow);
                        //    if (result != "success")
                        //    {
                        ...
                        //    }
                        //}
                        //if (step.Id == 8) //previously step 6 
                        //{
                        //    definition_step.Element("status").SetValue(1);
                        //    workflow.status = 23;
                        //    complete = 1;
                        //}
                        if (definition.Element("steps").Elements("step").Count() == 6)
                        {
                            if (step.Id == 2 || step.Id == 4 || step.Id == 5)
                            {
                                var result = new Procurement().Approve(definition, step, definition_step, collection, loggedInEmployee, id, workflow);
                                if (result != "success")
                                {
                ...
                else if (workflow.process_id.ToString().ToUpper() == "84E9FEBB-6C8E-42E2-B1A2-4DEE7F6E7317")
                {
                    ...
                    if (step.Id == 1)
                    {
                        procurement.AddItemsToPayment(definition, step, definition_step, collection, loggedInEmployee, id);
                        //upload supporting document
                ...
                //new procurement
                if (workflow.process_id.ToString().ToUpper().Equals("396E4066-81F1-4311-B076-FC3C53BB3177"))
                {//definition is the whole xml definition for the current process
                    NewProcurementProcess(loggedInEmployee, step, definition, collection, files);
                ...
                //reiembursment
                if (workflow.process_id.ToString().ToUpper().Equals("C1F128BE-EC3F-4163-B2C6-DCADCBEFD4FB"))
                {
                    //definition is the whole xml definition for the current process
                    ReimbursementRequest(loggedInEmployee, step, definition, collection, files, id, bLWorkflow, workflow);
                    if (ViewBag.message != "" && ViewBag.message != null)
                    {
                        return RedirectToAction("Step", new { id = id, message = ViewBag.message });
                    }
                ...
                //advance
                if (workflow.process_id.ToString().ToUpper().Equals("0FB8D2B5-54A1-4BC3-9020-EE7627638E59"))
                {
                    //definition is the whole xml definition for the current process
                    AdvanceRequest(loggedInEmployee, step, definition, collection, files, id, bLWorkflow, workflow);
                    if (ViewBag.message != "" && ViewBag.message != null)
                    {
                        return RedirectToAction("Step", new { id = id, message = ViewBag.message });
                    }
            ...
            if (collection["comment"] != null && collection["comment"] != "")
            {
                ...
                comment.Add(new XElement("employee_id", loggedInEmployee.emp_number));
                comment.Add(new XElement("info", collection["comment"].ToString()));
                comment.Add(new XElement("employee_name", string.Format("{0} {1}", loggedInEmployee.emp_lastname, loggedInEmployee.emp_firstname)));
            ...
            definition_step.Element("created_by").SetValue(loggedInEmployee.emp_number);
            definition_step.Element("created_by_name").SetValue(string.Format("{0} {1}", loggedInEmployee.emp_lastname, loggedInEmployee.emp_firstname));
            ...
            notification_email.to = new List<string> { loggedInEmployee.emp_work_email };
        ...
        private void process_document(int step, bpm_workflow workflow, XElement definition)
        {
            ...
            else if (workflow.process_id.ToString().ToUpper() == "C1F128BE-EC3F-4163-B2C6-DCADCBEFD4FB")
            {
                switch (step)
                {
                    /*case 5:
        ...
        [HttpPost]
        public ActionResult Tracker(DateTime start, DateTime end)
        {

            var ids = new Guid[] { Guid.Parse("2D008467-EA12-4821-A2F2-7568B76039F1"), Guid.Parse("94E9FEBB-6C8E-42E2-B1A2-4DEE7F6E7317"), Guid.Parse("10471746-0236-4F79-BE27-8AA1CBFF2D92"), Guid.Parse("396E4066-81F1-4311-B076-FC3C53BB3178") };

            ViewBag.workflows = _workflowService.GetWorkflows().Where(e => ids.Contains(e.process_id) && e.created_at >= start && e.created_at <= end);
            return View();
        }
        ...
        public ActionResult Reverse(Guid workflow_id, int step_id)
        {
            ...
            var workflow = bLWorkflow.GetWorkflow();
            workflow.status = 22;
            //Change status for color 
            foreach (var element in definition.Element("steps").Elements("step"))
            {
                if (Convert.ToInt16(element.Element("code").Value) > 18)
                {//   definition.Element("step").Element("status").SetValue("0");
                    element.Element("status").SetValue("0");
                }
            }
            ...
            workflow.workflow = definition.ToString();
            _workflowService.Update(workflow);
            return RedirectToAction("Step", new { id = workflow_id, step_id = step_id });
        }
        ...
        public void UploadQuotes(bpm_workflow workflow, FormCollection collection, XElement definition, Step step, XElement definition_step)
        {
            ...
            if (collection["vendor"] != null && collection["vendor"].ToString() != "")
            {
                ...
                foreach (var item in definition.Element("form").Element("items").Elements("item"))
                {
                    ...
                    {
                        item.Element("vendors_unit_price").SetValue(v_item.Element("unit_price").Value);
                        item.Element("vendors_total_price").SetValue(Convert.ToDouble(v_item.Element("unit_price").Value) * Convert.ToDouble(quantity));
                    }
                }
        ...
        public void generate_doc(Guid workflow_id)
        {
            ...
            else if (workflow.process_id.ToString().ToUpper() == "A9BE0559-94E4-4A6E-AE46-74A1344DF7EF")
            {
                ...
                //to cater for old flow,
                if (definition.Element("steps").Elements("step").Count() == 6)
                {
                    if (step.Id >= 6)//was previously 7, changed to 6 to allow for file generations
                    {
                        new WorkflowDocumentsHelper().createTravelAdvance(Server.MapPath("~/Documents/Travel/Advance/" + workflow.id + "_Advance" + ".pdf"), workflow.pm_project.code, definition.Element("form"), Server.MapPath("~/Documents/Images/reimbursement_header.png"), workflow.admin_hrm_employee, workflow.created_at, definition);
                ...
                {
                    if (step.Id >= 7)//was previously 7, changed to 6 to allow for file generations
                    {
                        new WorkflowDocumentsHelper().createTravelAdvance(Server.MapPath("~/Documents/Travel/Advance/" + workflow.id + "_Advance" + ".pdf"), workflow.pm_project.code, definition.Element("form"), Server.MapPath("~/Documents/Images/reimbursement_header.png"), workflow.admin_hrm_employee, workflow.created_at, definition);
            ...
            else if (workflow.process_id.ToString().ToUpper() == "C1F128BE-EC3F-4163-B2C6-DCADCBEFD4FB")
            {
                ...
                {
                    // authorization
                    new WorkflowDocumentsHelper().createRAuthorization(
                        Server.MapPath("~/Documents/Reimbursement/Authorization/" + workflow.id + "_Authorization" +
                                       ".pdf"), workflow.pm_project.code, definition.Element("form"),
                        Server.MapPath("~/Documents/Images/reimbursment_authorization_header.png"),
                        workflow.admin_hrm_employee, workflow.created_at, definition);

                    Util.AddWorkflowDocument(new bpm_workflow_document
                    {
                        location = "~/Documents/Reimbursement/Authorization/" + workflow.id + "_Authorization" + ".pdf",
                        name = "Reimbursement Authorization Form",
                        workflow_id = workflow.id
                    });
                }
                if (step.Id > 7)
                {
                    new WorkflowDocumentsHelper().createRF(Server.MapPath("~/Documents/Reimbursement/Request/" + workflow.id + "_Request" + ".pdf"), workflow.pm_project.code, definition.Element("form"), Server.MapPath("~/Documents/Images/reimbursment_request_header.png"), workflow.admin_hrm_employee, workflow.created_at, definition);

                    Util.AddWorkflowDocument(new bpm_workflow_document { location = "~/Documents/Reimbursement/Request/" + workflow.id + "_Request" + ".pdf", name = "Reimbursement Request Form", workflow_id = workflow.id });
                }
            }

            //advance
            else if (workflow.process_id.ToString().ToUpper() == "0FB8D2B5-54A1-4BC3-9020-EE7627638E59")
            {
                if (step.Id > 2)
                {

                    // authorization
                    new WorkflowDocumentsHelper().createAAuthorization(
                        Server.MapPath("~/Documents/Advance/Authorization/" + workflow.id + "_Authorization" +
                                       ".pdf"), workflow.pm_project.code, definition.Element("form"),
                        Server.MapPath("~/Documents/Images/advance_authorization_header.png"),
                        workflow.admin_hrm_employee, workflow.created_at, definition);

                    Util.AddWorkflowDocument(new bpm_workflow_document
                    {
                        location = "~/Documents/Advance/Authorization/" + workflow.id + "_Authorization" + ".pdf",
                        name = "Advance Authorization Form",
                        workflow_id = workflow.id
                    });
                }
                if (step.Id > 7)
                {
                    new WorkflowDocumentsHelper().createRF(Server.MapPath("~/Documents/Advance/Request/" + workflow.id + "_Request" + ".pdf"), workflow.pm_project.code, definition.Element("form"), Server.MapPath("~/Documents/Images/advance_request_header.png"), workflow.admin_hrm_employee, workflow.created_at, definition);

                    Util.AddWorkflowDocument(new bpm_workflow_document { location = "~/Documents/Advance/Request/" + workflow.id + "_Request" + ".pdf", name = "Advance Request Form", workflow_id = workflow.id });
                }
            }
        }
                ...
                else
                {
                    switch (nxt_step.Definition.Element("position").Attribute("pos_type").Value)
                    {
                        ...
                        case "jti":
                            ...
                            if (job_title_id == 4051 && mail_list.Count < 1)
                            {
                                var emp = _employeeService.GetEmployee(4340);
                                _workflowService.AddUserToWorkflow(new bpm_workflow_employee { emp_number = emp.emp_number, workflow_id = id, pending = 1, created_at = DateTime.Now });
                                mail_list.Add(emp.emp_work_email);
                            }
#if DEBUG
        ...
        {
            if (workflow.process_id.ToString().ToUpper() == "C7902F4A-36DD-4FDC-9B0C-B7059034010D")
            {
                switch (step)
                {
                    case 6:
            ...
            else if (workflow.process_id.ToString().ToUpper() == "C1F128BE-EC3F-4163-B2C6-DCADCBEFD4FB")
            {
                if (step > 7)
                {
                    new WorkflowDocumentsHelper().createRF(
                        Server.MapPath("~/Documents/Reimbursement/Request/" + workflow.id + "_Request" + ".pdf"),
                        workflow.pm_project.code, definition.Element("form"),
                        Server.MapPath("~/Documents/Images/reimbursment_request_header.png"),
                        workflow.admin_hrm_employee, workflow.created_at, definition);

                    Util.AddWorkflowDocument(new bpm_workflow_document
                    {
                        location = "~/Documents/Reimbursement/Request/" + workflow.id + "_Request" + ".pdf",
            ...
            FormCollection collection, HttpPostedFileBase[] files)
        {
            ...
            switch (stepId)
            {
                ...
                case (int)NewProcurementProcessTag.UploadQuotation:
                    ...
                    SendEmailForApproval(email_body, notification_email, workflow, id, employee, next_step_class, step, collection, definition, MailType.Attention);
                    break;
                case (int)NewProcurementProcessTag.HqApproval:
                    HQApproval(workflow, collection, definition, step, current_step_element_tag, employee);
                    SendEmailForApproval(email_body, notification_email, workflow, id, employee, next_step_class, step, collection, definition, MailType.Attention);
                    break;
                case (int)NewProcurementProcessTag.BidComparism:
                    BidComparison(workflow, collection, definition, employee, step, current_step_element_tag);
                    SendEmailForApproval(email_body, notification_email, workflow, id, employee, next_step_class, step, collection, definition, MailType.Attention);
                    break;
                ...
                case (int)NewProcurementProcessTag.ThirdCountryDirectorApproval:
                    ...
                    break;
                case (int)NewProcurementProcessTag.Complete:
                    new_procurement.Complete(definition, step, current_step_element_tag, collection, employee, id, workflow);
                    ViewBag.message = "This request has been marked as completed";
                    complete = 1;
                    break;
                default:
                    result = new_procurement.Approve(definition, step, current_step_element_tag, collection, employee, id, workflow);
        ...
        private void ReimbursementRequest(HRM.DAL.Models.admin_hrm_employee employee, Step step, XElement definition,
            FormCollection collection, HttpPostedFileBase[] files, Guid id, BLWorkflow blWorkflow, bpm_workflow workflow)
        {
            var travel_advance = new NewTravelProcess();
            XElement definition_step = null;
            definition_step = definition.Element("steps").Elements("step").FirstOrDefault(e => e.Element("code").Value == step.Definition.Element("code").Value);
            var complete = 0;

            //Get the next step
            Step next_step_class = null;
            if (!string.IsNullOrEmpty(step.Definition.Element("next_step").Value))
                next_step_class = blWorkflow.GetStep(Convert.ToInt32(step.Definition.Element("next_step").Value));
            ...
            {
                //travel authorization
                case (int)ReimbursementProcessTag.Authorization:
                    travel_advance.ReimbursementAuthorization(definition, collection, employee, id, step, definition_step);
                    if (Request.Files.Count > 0)
                    {
                        try
                        {
                            var file_path = "~/Documents/Reimbursement/SupportDocs/";
                            //var files = Request.Files;
                            foreach (HttpPostedFileBase file in files)
                            {
                                var location = Path.Combine(file_path, workflow.title + "_" + file.FileName.Replace(@"\", string.Empty));
                    ...
                    break;
                case (int)ReimbursementProcessTag.Reimbursement:
                    travel_advance.Reimbursement(definition, collection, employee, id, step, definition_step);
                    SendEmailForApproval(email_body, notification_email, workflow, id, employee, next_step_class, step, collection, definition, MailType.Approval);
                    break;
                case (int)ReimbursementProcessTag.Complete:
                    definition_step.Element("status").SetValue(1);
                    workflow.status = 23;
                    complete = 1;
                    //SendEmailForApproval(email_body, notification_email, workflow, id, employee, next_step_class, step, collection, definition, MailType.Approval);
                    break;
                //case (int)ReimbursementProcessTag.UnitHeadTravelAdvanceApproval:
                //    definition_step.Element("status").SetValue(1);
                //    definition.Element("next_step").SetValue(step.Definition.Element("next_step").Value);
                //    //workflow.status = 23;
                //    //complete = 1;
                //    SendEmailForApproval(email_body, notification_email, workflow, id, employee, next_step_class, step, collection, definition, MailType.Approval);
                //    break;
                //case (int)NewTravelAdvanceProcessTag.UnitHeadApproval:
                //    result = new Procurement().Approve(definition, step, definition_step, collection, employee, id, workflow);
                //    if (result != "success")
                //    {
                //        ViewBag.message = result;
                //        return;
        ...
        private void AdvanceRequest(HRM.DAL.Models.admin_hrm_employee employee, Step step, XElement definition,
           FormCollection collection, HttpPostedFileBase[] files, Guid id, BLWorkflow blWorkflow, bpm_workflow workflow)
        {
            var travel_advance = new NewTravelProcess();
            XElement definition_step = null;
            ...
            {
                //travel authorization
                case (int)AdvanceProcessTag.AdvanceRequest:
                    travel_advance.AdvanceRequest(definition, collection, employee, id, step, definition_step);
                    if (Request.Files.Count > 0)
                    {
                        try
                        {
                    ...
                    break;
                case (int)AdvanceProcessTag.Advance:
                    travel_advance.Reimbursement(definition, collection, employee, id, step, definition_step);
                    SendEmailForApproval(email_body, notification_email, workflow, id, employee, next_step_class, step, collection, definition, MailType.Approval);
                    break;
                case (int)AdvanceProcessTag.Complete:
                    definition_step.Element("status").SetValue(1);
                    workflow.status = 23;
                    complete = 1;
                    //SendEmailForApproval(email_body, notification_email, workflow, id, employee, next_step_class, step, collection, definition, MailType.Approval);
